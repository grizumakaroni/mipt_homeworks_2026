...
class LFUPolicy(Policy[K]):
    ...
    def register_access(self, key: K) -> None:
        current_counter = self._key_counter.pop(key, 0)
    ...
    def key_to_evict(self) -> K | None:
        ...
        newest_key = next(reversed(self._key_counter))
        candidates = [key for key in self._key_counter if key != newest_key]
        if not candidates:
            candidates = list(self._key_counter)

        key_to_evict = candidates[0]
        min_counter = self._key_counter[key_to_evict]
        for key in candidates[1:]:
            counter = self._key_counter[key]
...
class CachedProperty[V]:
    ...
    def __get__(self, instance: HasCache[Any, Any] | None, owner: type) -> V:
        ...
        if instance.cache.exists(self._cache_key):
            return cast("V", instance.cache.get(self._cache_key))
